...
(function () {
  ...
  var ART = {
    ...
    /* Mini cake in an open kraft box, three-quarter top view */
    bento: function (tone) {
      var c = tones(tone);
      return '' +
        '<img src="./assets/images/b-cake.webp" alt="Mini cake" class="category-cake-img" />';
    },
  ...
  var ICONS = {
    ...
    message: icon('<path d="M4 5h16v11H9l-4 3.5V16H4z"/>'),
  ...
  window.BK_ART = {
     ...
     *   1. the filename listed in BK_IMAGES for this id
    ...
    photo: function (id, alt, categoryId) {
      ...
      return '<img src="' + list[0] + '"' +
        ...
        ' data-bk-id="' + id + '"' +
        ' data-bk-next="' + list.slice(1).join('|') + '">';
    },
  ...
  document.addEventListener('error', function (e) {
    var img = e.target;
    if (!img || img.tagName !== 'IMG' || !img.hasAttribute('data-bk-next')) return;

    var next = (img.getAttribute('data-bk-next') || '').split('|').filter(Boolean);
    var id = img.getAttribute('data-bk-id') || '';

    if (next.length) {
      img.setAttribute('data-bk-next', next.slice(1).join('|'));
      img.src = next[0];
      if (!reported[id]) {
        reported[id] = true;
        if (window.console && console.info) {
          console.info('[Rosewood Bakery] No picture found for "' + id +
            '" — falling back. Add ' + id + '.webp to assets/images/ to fix it.');
        }
      }
    } else {
      // nothing left to try: hide the broken icon rather than show it
      img.removeAttribute('data-bk-next');
      img.style.display = 'none';
      if (window.console && console.warn) {
        console.warn('[Rosewood Bakery] No picture at all for "' + id + '".');
      }
    }
  }, true);
