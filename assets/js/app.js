...
(function () {
  ...
  var PRODUCTS = window.BK_PRODUCTS || [];
  var CATEGORIES = window.BK_CATEGORIES || [];
  var ART = window.BK_ART;
  ...
  /**
   * Format a price using currencySymbol + locale from data.js.
   * Whole numbers stay whole (38 -> $38); anything else gets two decimals
   * (4.5 -> $4.50), which is what shoppers expect.
   */
  function money(n) {
    n = Number(n);
    var digits = (n % 1 === 0) ? 0 : 2;
    return (CFG.currencySymbol || '$') + n.toLocaleString(CFG.locale || 'en-US', {
  ...
  /**
   * Build the order link with the message already typed out.
   * Which app opens is set by orderChannel in data.js:
   *   'whatsapp' -> WhatsApp    'sms' -> the phone's text app    'phone' -> dial
   */
  function orderLink(message) {
    var raw = String(CFG.phoneRaw || '').replace(/\D/g, '');
    if (CFG.orderChannel === 'phone') return 'tel:+' + raw;
    if (CFG.orderChannel === 'sms') {
      // "?&body=" is the one form both iOS and Android accept
      return 'sms:+' + raw + '?&body=' + encodeURIComponent(message);
    }
    return 'https://wa.me/' + raw + '?text=' + encodeURIComponent(message);
  }

  /** Icon that matches whichever order channel is switched on. */
  function orderIcon() {
    if (CFG.orderChannel === 'phone') return 'phone';
    if (CFG.orderChannel === 'sms') return 'message';
    return 'whatsapp';
  }
  ...
  function hydrateConfig() {
    ...
    $$('[data-link]').forEach(function (el) {
      ...
      else if (kind === 'order' || kind === 'whatsapp') {
        var msg = el.getAttribute('data-message') ||
          'Hello ' + CFG.name + '! I would like to place an order.';
        el.href = orderLink(msg);
        // 'phone' just dials, so opening a new tab would be wrong
        if (CFG.orderChannel === 'phone' || CFG.orderChannel === 'sms') {
    ...
    $$('[data-icon]').forEach(function (el) {
      ...
      // 'order' picks the glyph that matches CFG.orderChannel
      if (name === 'order' || name === 'whatsapp') name = orderIcon();
      el.innerHTML = ART.icon(name);
    });
      ...
      '<article class="product-card" data-reveal="rise">' +
        ...
        '<div class="product-card__body">' +
          ...
          '<div class="product-card__foot">' +
            ...
            '</span>' +
            '<a class="product-card__order" href="' + orderLink(message) + '" target="_blank" rel="noopener"' +
              ' aria-label="Order ' + esc(product.name) + '">' + ART.icon('bag') + '</a>' +
          '</div>' +
        '</div>' +
      '</article>';
  }
    ...
    return '' +
      '<a class="category-card" href="products.html#' + cat.id + '" data-reveal="rise">' +
        ...
        '<div class="category-card__foot">' +
          '<span class="category-card__price">from <b>' + money(cat.from) + '</b></span>' +
  ...
  function buildHero(heroCake) {
    var IMG = window.BK_IMAGES || {};
    var base = window.BK_IMAGE_BASE || 'assets/images/';
    ...
    heroCake.innerHTML = files.map(function (file, i) {
      return '<img src="' + base + file + '"' +
        ...
        (i === 0 ? '' : ' loading="lazy"') +
        ' decoding="async" data-bk-next="">';   // empty chain: hide if missing
    }).join('');
  ...
  /* ------------------------------------------------------------------------
     ...
     ------------------------------------------------------------------------ */
  function renderHome() {
    // Big home-page artwork: a photo if one is named in BK_IMAGES,
    ...
    if (values && window.BK_VALUES) {
      values.innerHTML = window.BK_VALUES.map(function (v) {
        return '' +
    ...
    if (story && window.BK_PROCESS) {
      story.innerHTML = window.BK_PROCESS.map(function (step, i) {
        return '' +
          '<div class="story-step">' +
            ...
            '<div class="story-step__art">' +
              // a photo if one is named in BK_IMAGES, otherwise the line drawing
              (IMG['step-' + (i + 1)]
    ...
    if (reviews && window.BK_TESTIMONIALS) {
      reviews.innerHTML = window.BK_TESTIMONIALS.map(function (t) {
        var stars = '';
  ...
  function renderProducts() {
    ...
    function draw() {
      ...
      if (!list.length) {
        grid.innerHTML = '' +
          '<div class="empty-state">' +
            ...
            '<a class="btn btn--gold" data-link="order" href="#">Ask us</a>' +
          '</div>';
        hydrateConfig();
      } else {
      ...
      if (window.BK_SCROLL) window.BK_SCROLL.refresh();
    }
  ...
  function renderFaq() {
    var wrap = $('#faqList');
    if (!wrap || !window.BK_FAQ) return;
    wrap.innerHTML = window.BK_FAQ.map(function (item, i) {
      return '' +
  ...
  function initOrderForm() {
        ...
        var opts = PRODUCTS.filter(function (p) { return p.category === cat.id; })
          .map(function (p) {
            return '<option value="' + esc(p.name) + '">' + esc(p.name) +
              ' — ' + money(p.price) + ' / ' + esc(p.unit) + '</option>';
          }).join('');
    ...
    form.addEventListener('submit', function (e) {
      ...
      window.open(orderLink(lines.join('\n')), '_blank', 'noopener');
      form.reset();
    });
