...
(function () {
  ...
  function injectStyles() {
    var css =
      '.bk-theme-switch{position:fixed;left:clamp(1rem,3vw,2rem);bottom:clamp(1rem,3vw,2rem);' +
      ...
      '.bk-theme-switch__label{font-size:.62rem;letter-spacing:.18em;text-transform:uppercase;' +
      'color:rgba(var(--cream-rgb),.6);padding-right:.2rem}' +
      '.bk-theme-switch button{width:26px;height:26px;border-radius:50%;cursor:pointer;padding:0;' +
      'border:2px solid transparent;transition:transform .3s,border-color .3s;position:relative}' +
      '.bk-theme-switch button:hover{transform:scale(1.15)}' +
      '.bk-theme-switch button[aria-pressed="true"]{border-color:var(--gold-soft);transform:scale(1.12)}' +
      '@media (max-width:520px){.bk-theme-switch__label{display:none}' +
      '.bk-theme-switch{left:.75rem;bottom:.75rem;padding:.45rem}}';
  ...
  function build() {
    ...
    box.className = 'bk-theme-switch';
    box.setAttribute('role', 'group');
    box.setAttribute('aria-label', 'Preview a colour theme');

    var label = document.createElement('span');
    label.className = 'bk-theme-switch__label';
